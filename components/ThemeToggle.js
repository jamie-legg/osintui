...
export default function ThemeToggle({ callback }) {
    ...
    return (
      ...
      className={classNames(
        darkActive ? 'bg-gray-800'  : 'bg-gray-200',
    ...
    >
      ...
      <span
        aria-hidden="true"
        className={classNames(
          darkActive ? 'translate-x-5' : 'translate-x-0',
          'pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200'
        )}
      >
                    <span className={classNames(darkActive ? "opacity-0" : "opacity-100", "light-switch absolute inset-0 h-full w-full flex items-center justify-center transition-opacity opacity-100 ease-out duration-100")}>
                    ...
                    <span className={classNames(darkActive ? "opacity-100" : "opacity-0", "dark-switch absolute inset-0 h-full w-full flex items-center justify-center transition-opacity opacity-0 ease-in duration-200")}>
